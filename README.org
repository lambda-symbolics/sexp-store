...
=revision-read= returns zero for absent or malformed revisions. Supply the
product's record tag and optional version to both operations.
...
Optional =:validate= adds domain constraints. =sidecar-write= accepts =:encode=
and the same token callbacks, declining stale publication. Source mutations must
increment the revision before writing source bytes.

=file-revision= returns pathname, byte size and write date. Combine these with
an explicit revision, since size and date cannot distinguish same-size edits
