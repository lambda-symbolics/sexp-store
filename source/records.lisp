...
(defun record--property-list-p (properties)
  "Return true when PROPERTIES is a complete property list."
...
(defun record-check (form &key tag versions fields (allow-other-keys t))
...
properties beyond :version and the described fields are rejected.
...
(defun snapshot-read-record
    (pathname &key tag versions fields (allow-other-keys t))
  "Read PATHNAME as exactly one versioned TAG record.

TAG, VERSIONS, FIELDS, and ALLOW-OTHER-KEYS are interpreted as by
RECORD-CHECK. Return the record form and its version. Signal
